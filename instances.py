...
class FrontikTestInstance(object):
    ...
    def start(self):
        ...
        supervisor.start_worker(script, app=self.app, config=self.config, port=self.port)
        wait_for(lambda: supervisor.is_running(self.port))

    def stop(self):
        if self.port is not None:
            supervisor.stop_worker(self.port)
            wait_for(lambda: not(supervisor.is_running(self.port)))
